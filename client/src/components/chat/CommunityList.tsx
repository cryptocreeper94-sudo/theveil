    ...
    <aside className="w-full sm:w-64 bg-slate-950/40 backdrop-blur-sm p-2 rounded-lg" data-testid="community-list">
      ...
      <div className="space-y-2">
        ...
        {communities.map((c) => (
          ...
          >
            <div className="h-10 w-10 rounded-full overflow-hidden bg-gradient-to-tr from-cyan-500 to-cyan-400 flex items-center justify-center text-black font-bold">
            ...
            {('unreadCount' in c && (c as any).unreadCount) ? <div className="px-2 py-1 text-xs bg-cyan-600 text-black rounded" data-testid={`community-unread-${c.id}`}>{(c as any).unreadCount}</div> : null}
          </motion.button>
        ))}
      </div>
    </aside>
